...
class MPCConfig:
    ...
    Q: np.ndarray               # 8x8 diagonal
    R: np.ndarray               # 4x4 diagonal
...
def parse_subtasks(raw: dict) -> list[Subtask]:
    ...
    valid_types = {"navigate", "hold_position", "return_home"}
    result = []
...
def parse_mpc_config(raw: dict) -> MPCConfig:
    ...
    # Weights -> build diagonal matrices
    w = raw["weights"]
    Q_pos = float(w["Q_pos"])
    Q_yaw = float(w["Q_yaw"])
    Q_vel = float(w["Q_vel"])
    ...
    Q = np.diag([Q_pos, Q_pos, Q_pos, Q_yaw, Q_vel, Q_vel, Q_vel, Q_vel])
    R = np.diag([R_lin, R_lin, R_lin, R_rot])
